...
1. **Public sites** — static HTML/CSS generated by Next.js, served by Nginx. No login, no admin links, no hint of a CMS. Just gorgeous portfolio/resume content. Served at the main domain (e.g. `yourdomain.com` for portfolio, `yourdomain.com/s/<slug>` for targeted sites).
2. **Admin app** — React SPA for document management, profile editing, and site generation. Lives at a separate subdomain (e.g. `app.yourdomain.com`). This is where login, uploads, and all management happens.
...
- `GET /` — Portfolio site (main domain, e.g. `yourdomain.com`)
- `GET /s/:slug` — Targeted site (e.g. `yourdomain.com/s/abc123`)
...
- Traefik labels route two subdomains:
  - `app.domain.com` → admin app (Nginx + React SPA)
  - `domain.com` → public sites (Nginx serving static files)
