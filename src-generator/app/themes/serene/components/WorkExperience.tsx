...
export default function WorkExperience({ experiences }: WorkExperienceProps) {
  ...
  return (
    <section id="experience" className="bg-serene-900">
      <div className="section-container">
        <h2 className="section-title text-gray-800">Work Experience</h2>
            ...
            <div key={index} className="bg-white rounded-xl p-6 border border-serene-700 hover:border-accent-teal-serene transition-colors shadow-sm">
              <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-4">
                <div>
                  ...
                  <p className="text-xl text-accent-teal-serene">{exp.company}</p>
                </div>
                ...
                <ul className="space-y-2 text-gray-700">
                  {exp.responsibilities.map((resp, idx) => (
                    <li key={idx} className="flex items-start">
                      <span className="text-accent-teal-serene mr-2 mt-1">▹</span>
                      <span>{resp}</span>
                    </li>
                  ))}
                </ul>
