...
export default function WorkExperience({ experiences }: WorkExperienceProps) {
  ...
  return (
    <section id="experience" className="bg-coral-900">
      <div className="section-container">
        <h2 className="section-title text-gray-900">Work Experience</h2>
        <div className="space-y-8">
          {experiences.map((exp, index) => (
            <div key={index} className="bg-white rounded-xl p-6 border-2 border-coral-700 hover:border-accent-coral-red transition-colors shadow-sm">
              <div className="flex flex-col md:flex-row md:justify-between md:items-start mb-4">
                <div>
                  <h3 className="text-2xl font-bold text-gray-900 mb-1">
                    {exp.title}
                  </h3>
                  <p className="text-xl text-accent-coral-red">{exp.company}</p>
                </div>
                ...
                <ul className="space-y-2 text-gray-700">
                  {exp.responsibilities.map((resp, idx) => (
                    <li key={idx} className="flex items-start">
                      <span className="text-accent-coral-red mr-2 mt-1">▹</span>
                      <span>{resp}</span>
                    </li>
                  ))}
                </ul>
